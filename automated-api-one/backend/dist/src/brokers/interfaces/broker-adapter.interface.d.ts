import { SessionResponse, OrderRequest, OrderResponse, PositionResponse, HoldingResponse, FundsResponse, BrokerHealthResponse, BrokerCapabilities, BrokerTrade, BrokerLtp, BrokerCallbackData, BrokerSession } from './broker-client.interface';
export declare abstract class BrokerAdapter {
    ...
    abstract refreshSession(token: string, refreshToken: string): Promise<SessionResponse>;
    abstract healthCheck(): Promise<BrokerHealthResponse>;
    abstract capabilities(): BrokerCapabilities;
    abstract modifyOrder(token: string, clientCode: string, orderId: string, variety: string, order: {
        quantity: number;
        price?: number;
        ordertype?: string;
        producttype?: string;
    ...
    abstract cancelOrder(token: string, clientCode: string, orderId: string, variety: string): Promise<OrderResponse>;
    abstract getTradeBook(token: string, clientCode: string): Promise<BrokerTrade[]>;
    abstract getLtpData(token: string, exchange: string, symbol: string, symbolToken: string): Promise<BrokerLtp>;
    abstract getOrderDetails(token: string, clientCode: string, orderId: string): Promise<OrderResponse>;
}
